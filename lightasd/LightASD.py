...
class LightASD(nn.Module):
    ...
    def train_network(self, loader, epoch, **kwargs):
        ...
        for num, (audioFeature, visualFeature, labels) in enumerate(loader, start=1):
            ...
            audioEmbed = self.model.forward_audio_frontend(
                audioFeature[0].to(self.device))
            visualEmbed = self.model.forward_visual_frontend(
                visualFeature[0].to(self.device))
    ...
    def evaluate_network(self, loader, evalCsvSave, evalOrig, **kwargs):
        ...
        mAP = float(str(subprocess.run(cmd, shell=True, stdout=PIPE,
                    stderr=PIPE).stdout).split(' ')[2][:5])
        return mAP

    def evaluate_single(self, audioFeature, visualFeature):
        """
        Evaluate a single audio-visual pair and return prediction scores.

        Args:
            audioFeature: Audio feature tensor
            visualFeature: Visual feature tensor

        Returns:
            predScore: Prediction scores
        """
        self.eval()
        with torch.no_grad():
            audioEmbed = self.model.forward_audio_frontend(
                audioFeature.to(self.device))
            visualEmbed = self.model.forward_visual_frontend(
                visualFeature.to(self.device))
            outsAV = self.model.forward_audio_visual_backend(
                audioEmbed, visualEmbed)
            # Use the labels=None case which directly returns prediction scores
            predScore = self.lossAV.forward(outsAV, labels=None)
...
def setup(device='cpu'):
    args = {
        'model_path': 'lightasd/weight/pretrain_AVA_CVPR.model',

        # ASD model parameters
        'lr': 0.001,
        'lrDecay': 0.95,
        'maxEpoch': 30,
        'testInterval': 1,
        'batchSize': 2000,
        'nDataLoaderThread': 64,
        'dataPathAVA': "AVADataPath",
        'savePath': "exps/exp1",
        'evalDataType': "val"
    }
